"""Semantic archetype classification for Serie A meio-campistas (Wyscout + SofaScore)."""
...
ARCHETYPE_LABELS = ("Contenção", "Construtor", "Box-to-box", "Híbrido")
...
HYBRID_BADGE_BY_PAIR: dict[frozenset[str], tuple[str, str]] = {
    frozenset({"Contenção", "Construtor"}): ("Volante Base", "Base"),
    frozenset({"Contenção", "Box-to-box"}): ("MC Combativo", "Combativo"),
    frozenset({"Construtor", "Box-to-box"}): ("MC Projetivo", "Projetivo"),
}
HYBRID_BADGE_TRIPLE = ("MC Completo", "Completo")
...
def _strong_axes(z_def: float, z_con: float, z_b2b: float, thr: float = HYBRID_Z_THRESHOLD) -> list[str]:
...
def _hybrid_badge(strong: list[str], z_def: float, z_con: float, z_b2b: float) -> tuple[str | None, str | None]:
    if len(strong) < 2:
        return None, None
    if len(strong) >= 3:
        return HYBRID_BADGE_TRIPLE
    zmap = {"Contenção": z_def, "Construtor": z_con, "Box-to-box": z_b2b}
...
def _primary_archetype(z_def: float, z_con: float, z_b2b: float) -> str:
    zmap = {"Contenção": z_def, "Construtor": z_con, "Box-to-box": z_b2b}
    primary = max(zmap, key=zmap.get)
    if primary == "Construtor" and (z_con < CONSTRUCTION_Z_THRESHOLD or z_b2b >= z_con):
        return "Box-to-box" if z_b2b >= z_def else "Contenção"
    return primary
...
def _branch_scores(feat_df: pd.DataFrame) -> pd.DataFrame:
    return pd.DataFrame(
        {
            "Contenção": _axis_z(feat_df, DEF_COLS),
            "Construtor": _axis_z(feat_df, CON_COLS),
            "Box-to-box": _axis_z(feat_df, B2B_COLS),
        },
        index=feat_df.index,
...
def build_mc_feature_df(pool: pd.DataFrame) -> pd.DataFrame:
    out = pool.copy()
    ...
    if "interception_won_p90" not in out.columns:
        out["interception_won_p90"] = pd.to_numeric(out.get("Interseções/90", out.get("Interseções")), errors="coerce").fillna(0)
    for col in dict.fromkeys(DEF_COLS + CON_COLS + B2B_COLS):
        out[col] = pd.to_numeric(out.get(col), errors="coerce").fillna(0.0)
    unique_cols = list(dict.fromkeys(DEF_COLS + CON_COLS + B2B_COLS))
...
def _classify_archetypes(feat_df: pd.DataFrame) -> pd.DataFrame:
    ...
    z_b2b = _axis_z(feat_df, B2B_COLS)
    ...
    for idx in feat_df.index:
        ...
        zb = float(z_b2b.loc[idx])
        strong = _strong_axes(zd, zc, zb)
        if len(strong) >= 2:
            primary = "Híbrido"
            badge, short = _hybrid_badge(strong, zd, zc, zb)
        else:
            primary = _primary_archetype(zd, zc, zb)
    ...
    return pd.DataFrame(
        {
            ...
            "cluster_share_contencao": shares["Contenção"].round(1),
            "cluster_share_construtor": shares["Construtor"].round(1),
            "cluster_share_boxtobox": shares["Box-to-box"].round(1),
            "mc_z_cont": z_def,
            "mc_z_con": z_con,
            "mc_z_b2b": z_b2b,
        },
...
def apply_mc_hierarchical_clusters(pool: pd.DataFrame) -> pd.DataFrame:
    """Add cluster_archetype, hybrid badge and per-axis share columns."""
    out = pool.copy()
    feat_df = build_mc_feature_df(out)
